...
public class PlanningOpstellenPanel extends javax.swing.JPanel implements PlanningObserver {
    ...
    private String geselecteerdePromotor;
    private String geselecteerdeCampus;
    private String geselecteerdLokaal;
    private List<String> geselecteerdePresentatie;
    ...
    private void verwijderPresentatie(ActionEvent evt) {
        ...
        if (dc.geefPresentatie(dag, tijdsvak) != null) {
            geselecteerdePresentatie = dc.geefPresentatie(dag, tijdsvak);
            dc.verwijderPresentatie(geselecteerdePresentatie.get(2));
            clearCalendar();
            tabelheadingInstellen();
        } else {
            JOptionPane.showMessageDialog(null, "Gelieve een geldige tijdsvak te kiezen.");
    ...
    private void setLokaalCombobox(JComboBox comboBox, String campusNaam) {
        comboBox.removeAllItems();
        for (String s : dc.geefLokalenByCampusId(campusNaam)) {
            comboBox.addItem(s);
        }
    }
    ...
    private void zetGeplandePresentaties() {
        ...
        Map<String, List<String>> presentaties = dc.geefPresentatiesByLokaalId(geselecteerdLokaal);
        for (Map.Entry<String, List<String>> p : presentaties.entrySet()) {
            int dag = Integer.parseInt(p.getValue().get(3));
            int tijdsvak = Integer.parseInt(p.getValue().get(4));
            String sEmail = p.getKey();
            List<String> student = dc.geefStudentByEmail(sEmail);
            String naam = student.get(0) + " " + student.get(1);
         
            planningModel.setValueAt(naam, tijdsvak, dag);
        }
    ...
    private void cmbCampusItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cmbCampusItemStateChanged
        if (evt.getItem() instanceof String) {
            geselecteerdeCampus = (String) evt.getItem();
            if (geselecteerdeCampus != null) {
                String cNaam = geselecteerdeCampus;
    ...
    private void btnKenJuryLidToeActionPerformed(java.awt.event.ActionEvent evt) {
        if (tblWeek.getSelectedColumnCount() > 0) {
            ...
            toekennenJuryledenDialog = new JuryledenToekennenJDialog(dc, geselecteerdePresentatie, geselecteerdePresentatie.get(2));
    ...
    private void btnPlanningBewerkenActionPerformed(java.awt.event.ActionEvent evt) {
        if (tblWeek.getSelectedColumnCount() > 0) {
            ...
            bewerkPlanningDialog = new PlanningBewerkenJDialog(dc, geselecteerdePresentatie, geselecteerdeCampus);
    ...
    private void cmbPromotorenItemStateChanged(java.awt.event.ItemEvent evt) {
        ...
        for (Map.Entry<String, String> promotor : promotoren.entrySet()) {
            String email = promotor.getKey();
            String naam = promotor.getValue();
            if (naam.equals(evt.getItem())) {
                renderer.setPromotorEmail(email); //id meegeven van promotor.
                this.repaint();
                geselecteerdePromotor = email;
            }
        }
    }
    ...
    private void btnPlanPresentatieActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (tblWeek.getSelectedColumn() > 0 && dc.geefPresentatie(dag, tijdsvak) == null) {
            Collection<List<Integer>> beschikbaarheden = dc.geefBeschikbaarhedenVanPromotor(geselecteerdePromotor).values();
            if (beschikbaarheden.isEmpty()) {
                JOptionPane.showMessageDialog(null, "De geselecteerde promotor heeft geen beschikbaarheden.");
            }
            for (List<Integer> info: dc.geefBeschikbaarhedenVanPromotor(geselecteerdePromotor).values()) 
            {
                int bDag = info.get(0);
                int bTijdsvak = info.get(1);
                if (dag == bDag && tijdsvak == bTijdsvak) 
                { 
                    for (Map.Entry<String, String> p : dc.geefPromotoren().entrySet()) 
                    {
                         if (cmbPromotoren.getSelectedItem().equals(p.getValue())) 
                         {
                            geselecteerdePromotor = p.getKey();
                         }
                    }
                    
                    geselecteerdLokaal = (String) cmbLokaal.getSelectedItem();
                    geselecteerdeCampus = (String) cmbCampus.getSelectedItem();
                    planPresentatieDialog = new PlanPresentatieJDialog(dc, geselecteerdePromotor, 
                            geselecteerdeCampus, geselecteerdLokaal, dag, tijdsvak);
            ...
            if (!gelukt) {
                JOptionPane.showMessageDialog(null, "De geselecteerde promotor kan hier niet plannen.");
            }
